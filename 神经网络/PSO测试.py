...
# 记录初始均方误差
initial_mse = objective_function([0.1, 0.0001])
mse_values.append(initial_mse)

# 输出进化过程的图
def plot_mse_evolution(mse_values):
    plt.figure(figsize=(8, 5))
    plt.plot(range(len(mse_values)), mse_values, marker='o', linestyle='-', color='b')
    plt.xlabel('迭代次数')
    plt.ylabel('均方误差')
    plt.title('PSO-BP模型均方误差随迭代次数的变化')
    plt.grid(True)
    plt.show()
