...
plt.xlabel('实际值')
plt.ylabel('预测值')
plt.title('随机森林模型验证散点图\nR2 = {:.4f}'.format(r2))

# 输出图片到指定路径
output_image_folder = r'D:\矫正完成夜间灯光数据连续\西北\新疆维吾尔自治区\碳排放\RandomForest\xlsx'
os.makedirs(output_image_folder, exist_ok=True)
output_image_path = os.path.join(output_image_folder, 'RandomForest_Model_Validation.png')
...
# 创建输出xlsx表格
output_xlsx_path = os.path.join(output_image_folder, 'RandomForest_Model_Validation.xlsx')
...
for year in range(2000, 2021):
    ...
    if tif_file is not None:
        ...
        # 使用随机森林模型预测每个像元点的碳排放量
        carbon_emissions = random_forest.predict(input_data)
        ...
        # 将原本DN值为0的像元值的预测结果设置为0
        #carbon_emissions[dn_values == 0] = 0
        ...
        # 创建输出栅格图像并将预测结果写入其中
        output_folder = r'D:\矫正完成夜间灯光数据连续\西北\新疆维吾尔自治区\碳排放\RandomForest'
