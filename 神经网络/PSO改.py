...
def objective_function(weights):
    mlp = MLPRegressor(hidden_layer_sizes=(5,), activation='relu', max_iter=50,
...
final_mlp = MLPRegressor(hidden_layer_sizes=(5,), activation='relu', max_iter=50,
...
# 对预测值y_pred进行逆归一化还原到原始标签空间
y_pred_original = scaler_y.inverse_transform(y_pred.reshape(-1, 1)).flatten()

# 对测试集的真实值y_test进行逆归一化还原到原始标签空间
...
# 绘制验证结果的折线图
plt.subplot(1, 2, 1)
plt.plot(y_test_original, label='实际值', color='blue', marker='o')
plt.plot(y_pred_original, label='预测值', color='red', marker='x')
plt.xlabel('样本序号')
plt.ylabel('碳排放量')
...
# 输出图片到指定路径
output_image_folder = r'D:\矫正完成夜间灯光数据连续\西北\新疆维吾尔自治区\碳排放\PSO\xlsx'
...
df_validation = pd.DataFrame({'实际值': y_test_original, '预测值': y_pred_original})
...
for year in range(2000, 2021):
    ...
    if tif_file is not None:
        ...
        # 对预测结果进行逆归一化还原到原始标签空间
        carbon_emissions_original = scaler_y.inverse_transform(carbon_emissions.reshape(-1, 1)).flatten()

        # 计算有效像元的最小值
        min_value = np.nanmin(carbon_emissions_original)

        # 将预测结果中的最小值减去所有像元值（排除Nodata值）
        carbon_emissions_original = carbon_emissions_original - min_value

        # 创建输出栅格图像并将预测结果写入其中
        predicted_array = np.ones(band.shape, dtype=np.float32) * band.fill_value
        predicted_array[~band.mask] = carbon_emissions_original

        # 创建输出栅格图像并将预测结果写入其中
        output_folder = r'D:\矫正完成夜间灯光数据连续\西北\新疆维吾尔自治区\碳排放\PSO'
