...
# 读取Excel表格数据
input_file = r'D:\矫正完成夜间灯光数据连续\东北\Excle\PSO辽宁.xlsx'
...
# 定义传统BP模型
bp_mlp = MLPRegressor(hidden_layer_sizes=(30,), activation='relu', max_iter=60,
                     random_state=42)

# 训练传统BP模型
bp_mlp.fit(X_train, y_train)

# 验证模型并绘制验证结果
y_pred = bp_mlp.predict(X_test)
...
plt.xlabel('实际值')
plt.ylabel('预测值')
plt.title('传统BP模型验证散点图\nR2 = {:.4f}'.format(r2))

# 输出图片到指定路径
output_image_folder = r'D:\矫正完成夜间灯光数据连续\东北\辽宁省\碳排放\BP\xlsx'
os.makedirs(output_image_folder, exist_ok=True)
output_image_path = os.path.join(output_image_folder, 'BP_Model_Validation.png')
...
# 创建输出xlsx表格
output_xlsx_path = os.path.join(output_image_folder, 'BP_Model_Validation.xlsx')
...
# 指定文件夹路径
folder_path = r'D:\矫正完成夜间灯光数据连续\东北\辽宁省'
...
for year in range(2000, 2021):
    ...
    if tif_file is not None:
        ...
        # 使用传统BP模型预测每个像元点的碳排放量
        carbon_emissions = bp_mlp.predict(input_data)
        ...
        # 创建输出栅格图像并将预测结果写入其中
        output_folder = r'D:\矫正完成夜间灯光数据连续\东北\辽宁省\碳排放\BP'
